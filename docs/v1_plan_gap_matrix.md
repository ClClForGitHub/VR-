...
| 8 | Blender assembly pipeline | Partial to mostly done | Existing-script compose/export/render path, live demo `.blend`, `viewer_scene.glb`, package, runtime `blender_results` handoff-apply into `BlenderSceneState`/`ViewerSceneState`, interactive `BLENDER_PREVIEW` approve-vs-edit user actions, a SceneSpec-driven non-dry-run compose/export/viewer-check run under `outputs/runs/20260629_scene_spec_assembly_non_dryrun/`, and UI28 deterministic assembly planning with composite front/back/left/right placement, `camera_target_normalized`, aspect-aware render resolution, compose-script camera target consumption, and targeted tests proving the SceneSpec workflow writes the richer plan | LLM/MCP assembly planner into actual Blender operations, stronger asset-aware layout/camera reasoning, and a new non-dry-run preview using the richer plan |
